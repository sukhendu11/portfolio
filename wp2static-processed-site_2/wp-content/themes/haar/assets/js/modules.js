...
(function($) {
    ...
    common.edgtfElementInView = edgtfElementInView;
    ...
    */
	...
	var edgtfInitAnchor = function() {
		...
		var checkActiveStateOnLoad = function(){
			...
			if(hash !== "" && $('[data-edgtf-anchor="'+hash+'"]').length > 0){
				...
				$(document).trigger('edgtfAnchorLoaded');
			}
		};
		...
		var anchorClickOnLoad = function ($this) {
			...
			if (hash !== '' && anchorData.length > 0) {
				...
				$(document).trigger('edgtfAnchorLoaded');
				edgtf.anchorOffset = anchoredElementOffset;
		...
		var anchorClick = function () {
			edgtf.document.on("click", ".edgtf-main-menu a, .edgtf-fullscreen-menu a, .edgtf-btn, .edgtf-anchor, .edgtf-mobile-nav a", function () {
				$(document).trigger('edgtfAnchorClicked');
	...
	function edgtfInitBackToTop() {
		...
		backToTopButton.on('click', function (e) {
			e.preventDefault();
			edgtf.html.animate({scrollTop: 0}, edgtf.window.scrollTop() / 5, 'easeInOutQuart');
		});
	}
                ...
                searchField.on('keyup paste', function() {
                    ...
                    keyPressTimeout = setTimeout( function() {
                        ...
                        if(searchTerm.length < 3) {
                            ...
                            var ajaxData = {
                                action: 'haar_edge_search_post_types',
                                term: searchTerm,
                                postType: postType,
    ...
    function edgtfOwlSlider() {
        ...
        if (sliders.length) {
            sliders.each(function(){
                ...
                if (sliderDataHolder.data('slider-padding') === 'yes' && slider.hasClass('edgtf-ig-slider')) {
                    stagePaddingEnabled = true;
                    stagePadding = parseInt(slider.outerWidth() * 0.347);
                    margin = 120;
                }
                ...
                if(slider.hasClass('edgtf-vertical-slides')) {
                    animateInClass = 'slideInUp';
                    animateOutClass = 'slideOutUp';
                }
                if(slider.hasClass('edgtf-vertical-slides') || slider.hasClass('edgtf-pag-on-slider')) {
                    navigation = false;
                }
                ...
                if(slider.hasClass('edgtf-ps-wide-slider')) {
                    stagePaddingEnabled = true;
                    stagePadding = parseInt(slider.outerWidth() * 0.1586);
                    margin = 0;
                    responsiveMargin1 = 0;
                    responsiveMargin = 0;
                }
	            ...
	            slider.waitForImages(function () {
		            owlSlider = slider.owlCarousel({
			            ...
			            nav: navigation,
			            navText: [
				            '<span class="edgtf-prev-icon ion-ios-arrow-back"></span>',
				            '<span class="edgtf-next-icon ion-ios-arrow-forward"></span>'
			            ],
			            responsive: {
				            0: {
			            ...
			            onInitialize: function () {
                            ...
                            if (slider.hasClass('edgtf-testimonials')) {
                            	edgtfElementInView(slider);
                            }
			            },
    ...
    * Init Images Tilt FX
    */
    function edgtfInitTiltFx() {
    	if (!edgtf.htmlEl.hasClass('touch')) {
    		//handle clicks
    		var tiltItems = $('.edgtf-tilt-item');

    		tiltItems.each(function() {
    			var tiltItem = $(this),
	    			tiltParent = tiltItem.closest('.edgtf-tilt-parent'),
	    			tiltLink = tiltParent.find('.edgtf-tilt-link');

	    		if (tiltLink.length) {
		    		tiltParent.on('click', function() {
			    		tiltLink[0].on('click');
		    		});
	    		}
    		});
    		...
    		//init tilt
	    	Array.from(document.querySelectorAll('.edgtf-tilt-item')).forEach(function(item) {
	    		var options = JSON.parse(item.getAttribute('data-tilt-options'));
				new TiltFx(item, options);
	    	});
    	}
    }
    ...
    */
    function edgtfElementInView(element) {
    	$(window).scroll(function(){
    		if (edgtf.scroll > element.offset().top - edgtf.windowHeight && edgtf.scroll < element.offset().top + element.height()) {
    			if (!element.hasClass('edgtf-in-view')) {
    				element.addClass('edgtf-in-view');
    			}
    		} else {
    			if (element.hasClass('edgtf-in-view')) {
    				element.removeClass('edgtf-in-view');
    			}
    		}
    	});
    }
	...
	function edgtfInitBlogPagination(){
		...
		var initMainPagFunctionality = function(thisHolder) {
			...
			if(nextPage <= maxNumPages){
				...
				var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'haar_edge_blog_load_more');
    ...
    function edgtfInitBlogListShortcodePagination(){
        ...
        var initMainPagFunctionality = function(thisHolder, pagedLink) {
            ...
            if(nextPage <= maxNumPages){
                ...
                var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'haar_edge_blog_shortcode_load_more');
