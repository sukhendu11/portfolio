		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'haar_edge_gallery_upload_get_images',
						ids: ids,
			...
			edgtfImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? 'haar_content_0' + i + '.xml' : 'haar_content_' + i + '.xml';
						...
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? str = 'haar_content_0' + i + '.xml' : 'haar_content_' + i + '.xml';
